...
int main(int argc, char *argv[]) {
    ...
    auto rays = CUDABuffer<Ray>::create(resolution * resolution);
    auto hits = CUDABuffer<Hit>::create(resolution * resolution);
    std::vector<Hit> host_hits(resolution * resolution);
    ...
    auto generate_volume = [&] {
        auto vox_res = 1u << voxelization_level;
        auto volume = Volume::from(*mesh, vox_res, camera.rotation_to_world());
    ...
    auto volume = generate_volume();
    auto octree = volume->cuda();

    Framerate framerate;
    window.run([&] {
        auto ray_origin = camera.position();

        cuda_camera_generate_rays(camera, world_to_object, rays->data(), resolution, resolution, static_cast<uint32_t>(framerate.count()));
        octree->trace_closest(rays->data(), hits->data(), resolution, resolution);
        hits->download(host_hits.data());
        ...
        // render volume: part 2
        cudaStreamSynchronize(nullptr);
        stream.dispatch_1d(resolution * resolution, [&](uint32_t tid) noexcept {
            auto hit = host_hits[tid];
            auto radiance = [&] {
        ...
        with_imgui_window("Console", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize, [&] {
            ...
            ImGui::SliderInt("Voxelization Level", reinterpret_cast<int *>(&voxelization_level), 1, 12, "%d", ImGuiSliderFlags_AlwaysClamp | ImGuiSliderFlags_ClampOnInput);
            ...
            ImGui::SameLine();
            if (ImGui::Button("Dump Volume")) { volume->dump("volume.obj"); }
        });
